...
router.post('/subscribe', function(req, res) {
    ...
    // Create payload
    const payload = JSON.stringify({title:'Time to leave for class!'});
...
router.post('/get_user', function(req, res){
	let email = req.body.email;
	mydb.findUserRecByEmail(email, function(result){
		console.log('success'); //remove this
...
router.post('/settings', function(req, res){
	...
	if(mode === 'u'){
		...
		mydb.updateNotifications(newRec, function(result){
			res.send(result);
		});
	}
});
...
router.post('/email', (req, res) => {
	// CardinalDirectNots@gmail.com
	// ?GoCards2022

	var transporter = nodemailer.createTransport({
		service: 'gmail',
		auth: {
			user: 'CardinalDirectNots@gmail.com',
			pass: '?GoCards2022'
		}
	});

	var mailOptions = {
		from: 'CardinalDirectNots@gmail.com',
		to: req.body.email,
		subject: 'Time to leave for class!',
